...
:::danger
需要注意的是：在函数定义的过程中如果需要传地址参数的话，尽量用``void *func(type *,type *,...)``来定义，而不是``void *func(type &,type &,...)``，当然在标准C语言中没有存在函数参数是``&``运算符，在C++中才有定义，然而这是国内数据结构和算法书籍的大多数写法或者说某些老师教学过程中把C与C++混为一谈，这是不可取的。另外一方面从逻辑上讲，试想以下，当你在阅读代码使用函数的情况下，不能根据外部输入判定我传进去的是什么类型参数，对于直觉来说是很不高效的。就比如说：
```cpp title="C++代码"
void func1(int *a)
{
	*a = 100;
...
*/
```
以上代码可以说我们清楚在main函数中我们清楚明了知道p值是指针传进去函数的也是指针，不用再翻找fun1()函数的具体定义，同样的如下代码：
```c++ title="C++语言"
void func2(int &a);
int main(){
	int p = 10;
...
*/
```
但就是我们从``main()``函数的变量定义可以看出，``p``就是``int``类型，不是指针，我传进去就是int类型，但是实际上你还要去翻找函数具体查看函数的抽象才能得知，这原来是吧我传进去的东西给取地址。。。很是感谢。。。
:::
...
以上调试输出为``[list_create] me->next->next = (nil)`` 即使你没有``me->next = malloc(sizeof(*me));``这个语句，程序也可能会执行成功(但是在申请结构体空间的时候，已经把``me->next``赋值为``nil``空指针)。但是，这是不安全的，因为你不能把握 ``me->next`` 是不是一个野指针。因此，建议为 me->next 分配内存空间。
:::tip
