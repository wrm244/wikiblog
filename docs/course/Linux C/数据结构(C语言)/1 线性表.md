...
以上调试输出为``[list_create] me->next->next = (nil)`` 即使你没有``me->next = malloc(sizeof(*me));``这个语句，程序也可能会执行成功(因为在申请结构体空间的时候，已经把``me->next``赋值为``nil``空指针可以从调试上看得出来)。但是，这是不安全的，因为你不能把握 ``me->next`` 是不是一个野指针。因此，建议为 me->next 分配内存空间。
:::tip
