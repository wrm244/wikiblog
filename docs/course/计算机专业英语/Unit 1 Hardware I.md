...
> 毫不夸张得讲，数以百计的不同类型的计算机被几十家制造商销售。每种类型的复杂性可能相差很大，但最终每个处理器(有时称为中央处理单元或CPU)只有两个基本部分:控制单元和算术与逻辑单元。主存储器在处理器的内部系统中也起着不可或缺的作用。主存储器、控制单元、算术和逻辑单元这三个一起工作。让我们看看它们的功能和它们之间的关系。
...
>6. state-of-the-art 最先进的技术、应用最新理念的(Advanced technology)

Primary storage, or main memory, provides the processor with **temporary** storage for programs and data. All programs and data must be transferred to primary storage from an input device (such as a VDT(Video Display Terminals) ) or from secondary storage (such as a disk ) before programs can be executed or data can be processed. Primary storage space is always at a **premium** ;therefore, after a program has been executed, the storage space it **occupied** is reallocated to another program awaiting execution.
>主存储器或内存为处理器提供程序和数据的临时存储器。在执行程序或处理数据之前，必须将所有程序和数据从输入设备(如VDT(视频显示终端))或从辅助存储器(如磁盘)传输到主存储器。主存储空间总是很宝贵的;因此，在程序执行结束之后，它所占用的存储空间被重新分配给等待执行的另一个程序。
...
Figure 1-1 illustrates how all input/output(I/O) is "read to" or "written from" primarystorage.

In the figure,an **inquiry** (input) is made on a VDT. The inquiry,in the form of a message,is routed to primary storage over a channel (such as a **coarial cable**). The message is **interpreted**,and the processor **initiates** action to **retrieve** the **appropriate** program and data from secondary storage, The program and data are "loaded", or moved,to primary storage from secondary storage. This is a **nondestructive** read process. That is,the program and data that are read **reside** in both primary storage (temporarily) and secondary storage(**permanently**).The data are **manipulated** according to program **instructions**, and a report is written from primary storage to a printer.
> 在图中，在VDT上进行了查询(输入)。查询以消息的形式通过信道(如同轴电缆)传送到到主存中。该消息被解释的同时处理器开始执行从二级存储器中检索适当的程序和数据的动作，程序和数据被从二级存储器“加载”或“移动”到主存储器。这是一个非破坏性的读取过程。也就是说，被读取的程序和数据同时驻留在主存储器(临时)和辅助存储器(永久)中。根据程序指令操作数据，并从主存储器向打印机写入报告。
> 1. [inquiry](http://mobile.youdao.com/dict?le=eng&q=inquiry) us: [ˈɪnkwəri; ɪnˈkwaɪəri] uk: [ɪnˈkwaɪəri] n. 询问，疑问；调查，审查； 探查，探询
> 2. coarial cable 同轴电缆
> 3. [interpreted](http://mobile.youdao.com/dict?le=eng&q=interpreted) us: [ɪnˈtɜːrprətɪd] uk: [ɪnˈtɜːprətɪd] v. 解释，说明；口译；把……理解为；演绎（interpret 的过去式和过去分词）
> 4. [initiates](http://mobile.youdao.com/dict?le=eng&q=initiates) us: [ɪˈnɪʃieɪts] uk: [ɪˈnɪʃieɪts] vt. 启动，开始（initiate 的第三人称单数）
> 5. [retrieve](http://mobile.youdao.com/dict?le=eng&q=retrieve) us: [rɪˈtriːv] uk: [rɪˈtriːv]v. 找回，收回；检索（储存于计算机的信息）；（狗等）衔回（物品、猎物）；挽救，挽回；回忆，追忆；收绕钓鱼线 n. （尤指中枪猎物的）找回；收绕钓鱼线；<古> 恢复的可能性；检索
> 6. [appropriate](http://mobile.youdao.com/dict?le=eng&q=appropriate) us: [əˈproʊpriət] uk: [əˈprəʊpriət] adj. 合适的，相称的 v. 拨出（款项）；私占，挪用
> 7. [nondestructive](http://mobile.youdao.com/dict?le=eng&q=nondestructive) us: [ˌnɑːndɪˈstrʌktɪv] uk: [ˈnɒndɪsˈtrʌktɪv] adj. 无损的；非破坏性的
> 8. [reside](http://mobile.youdao.com/dict?le=eng&q=reside) us: [rɪˈzaɪd] uk: [rɪˈzaɪd] v. <正式>居住，定居；<正式>**存在于**；<正式>（权利、权力等）属于；位（于）
> 9. [permanently](http://mobile.youdao.com/dict?le=eng&q=permanently) us: [ˈpɜːrmənəntli] uk: [ˈpɜːmənəntli] adv. 永久地；（问题或困难）不断发生地，一直存在地
> 10. [manipulated](http://mobile.youdao.com/dict?le=eng&q=manipulated) us: [məˈnɪpjuleɪtɪd] uk: [məˈnɪpjuleɪtɪd] v. 操作（manipulate 的过去分词）
> 11. [instructions](http://mobile.youdao.com/dict?le=eng&q=instructions) us: [ɪnˈstrʌkʃ(ə)nz] uk: [ɪnˈstrʌkʃənz] n. 指令；说明（instruction 的复数形式）
