...
description: AOP在不改原有代码的前提下对其进行增强
last_update:
  date: 2023/06/17
---
...
- 切入点(Pointcut):匹配连接点的式子
  - 在SpringAOP中，一个切入点可以描述一个具体方法，也可也匹配多个方法
    - 一个具体的方法:如com.wrm244.dao包下的BookDao接口中的无形参无返回值的save方法
...
```java
execution(void com.wrm244.dao.BookDao.update())
```

描述方式二：执行com.wrm244.dao.impl包下的BookDaoImpl类中的无参数update方法
...
```
execution(public User com.wrm244.service.UserService.findById(int))
```

- execution：动作关键字，描述切入点的行为动作，例如execution表示执行到指定切入点
- public:访问修饰符,还可以是public，private等，可以省略
- User：返回值，写返回值类型
- com.wrm244.service：包名，多级包使用点连接
- UserService:类/接口名称
- findById：方法名
...
- `*`:单个独立的任意符号，可以独立出现，也可以作为前缀或者后缀的匹配符出现
  ...
  ```
  execution（public - com.wrm244.*.UserService.find*(*))
  ```

  匹配com.wrm244包下的任意包中的UserService类或接口中所有find开头的带有一个参数的方法
...
运行后会报错，错误内容为:
```bash
Exception in thread "main" org.springframework.aop.AopInvocationException: Null return value from advice does not match primitive return type for: public abstract int com.wrm244.dao.BookDao.select()
...
- 切入点表达式标准格式：动作关键字(访问修饰符  返回值  包名.类/接口名.方法名（参数）异常名)
  ...
  ```
  execution(- com.wrm244.service.*Service.*(..))
  ```
...
该环境是基于转账环境来完成的，所以环境的准备可以参考`之前环境搭建步骤`，在其基础上，我们继续往下写
