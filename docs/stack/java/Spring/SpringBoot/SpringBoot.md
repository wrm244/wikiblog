...
在  `com.wrm244.controller` 包下创建 `BookController` ，代码如下：
...
运行 `SpringBoot` 工程不需要使用本地的 `Tomcat` 和 插件，只运行项目 `com.wrm244` 包下的 `Application` 类，我们就可以在控制台看出如下信息
...
在该工程中的 `com.wrm244.controller` 包下创建一个名为 `BookController` 的控制器。内容如下：
...
在 `com.wrm244.controller` 包写创建名为 `BookController` 的控制器，内容如下
...
在 `com.wrm244.domain` 包下创建一个名为 `Enterprise` 的实体类等会用来封装数据，内容如下
...
在 `com.wrm244.service` 下创建 `BookService` 接口，内容如下
...
在 `com.wrm244.service.impl` 包写创建一个 `BookServiceImpl` 类，使其实现 `BookService` 接口，内容如下
...
在 `test/java` 下创建 `com.wrm244` 包，在该包下创建测试类，将 `BookService` 注入到该测试类中
...
在 `com.wrm244.domain` 包下定义实体类 `Book`，内容如下
...
在 `com.wrm244.dao` 包下定义 `BookDao` 接口，内容如下
...
在 `test/java` 下定义包 `com.wrm244` ，在该包下测试类，内容如下
...
![](assets/image-20210917225019868.png)
...
![](assets/image-20210917225715519.png)
...
![](assets/image-20210917230702072.png)
