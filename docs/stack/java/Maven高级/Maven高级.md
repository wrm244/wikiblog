...
tags: [Java,Maven]  
keywords: [Java,Maven]
description: Maven的高级特性，分模块开发、继承聚合、私服等
last_update:
  date: 2023/06/28
---
...
在`maven_03_pojo`项目中创建`com.wrm244.domain`包，并将`maven_02_ssm`中Book类拷贝到该包中
...
在`maven_04_dao`项目中创建`com.wrm244.dao`包，并将`maven_02_ssm`中BookDao类拷贝到该包中
...
* 该命令可以不借助IDEA，直接使用cmd命令行进行跳过测试，需要注意的是cmd要在``pom.xml``所在目录下进行执行。
...
第5，6步骤是创建wrm244-snapshot仓库

第7，8步骤是创建wrm244-release仓库
...
<servers>
    <server>
        <id>wrm244-snapshot</id>
        <username>admin</username>
        <password>admin</password>
    </server>
    <server>
        <id>wrm244-release</id>
        <username>admin</username>
        <password>admin</password>
    </server>
</servers>
...
<distributionManagement>
    <repository>
        ...
        <id>wrm244-release</id>
         <!--release版本上传仓库的具体地址-->
        <url>http://localhost:8081/repository/wrm244-release/</url>
    </repository>
    <snapshotRepository>
        <!--和maven/settings.xml中server中的id一致，表示使用该id对应的用户名和密码-->
        <id>wrm244-snapshot</id>
        <!--snapshot版本上传仓库的具体地址-->
        <url>http://localhost:8081/repository/wrm244-snapshot/</url>
    </snapshotRepository>
</distributionManagement>
```
...
现在发布是在wrm244-snapshot仓库中，如果想发布到wrm244-release仓库中就需要将项目pom.xml中的version修改成RELEASE即可。
